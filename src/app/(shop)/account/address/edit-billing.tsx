...
import { useAddressStore, BillingAddress } from '../../../../store/address-store';

export default function EditBillingAddressScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams();
  const addressId = typeof id === 'string' ? parseInt(id, 10) : 0;
  
  const { user } = useAuthStore();
  const { billingAddresses, isLoading, error, updateBillingAddress } = useAddressStore();
  
  // Find the address to edit
  const addressToEdit = billingAddresses.find(a => a.BillingAddressId === addressId);
  ...
  // Populate form with address data when found
  useEffect(() => {
    if (addressToEdit) {
      ...
      // If we have an ID but no address, show an error
  ...
  async function handleUpdate() {
    ...
    try {
      const success = await updateBillingAddress({
